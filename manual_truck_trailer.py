...
class World(object):
    ...
    def restart(self):
        ...
        blueprint = random.choice(get_actor_blueprints(self.world, "vehicle.daf.dafxf", "2"))
        ...
        if self.player is not None:
            ...
            spawn_actor_point.location.y = 6389.307617
        ...
        while self.player is None:
            ...
            if testing:
                ...
                directory = '/home/daniel/data-rllib-integration/data/data_015eddbfe65/'
                collisions_file = open(directory + 'collisions', 'r')
                collision_lines = collisions_file.readlines()

                lidar_data_file = open(directory + 'lidar_data', 'r')
                lidar_data_lines = lidar_data_file.readlines()
                counter = 0
                for collision, lidar_data in zip(collision_lines,lidar_data_lines):
                    counter += 1
                    if collision != '[]\n' and counter > 2600:
                        splits = collision.split('\'')
                        vehicle = splits[1]
                        actor_transform_text = splits[3]
                        timestamp_collision = splits[7]

                        open_brackets_indices = find(lidar_data,'[')
                        closed_brackets_indices = find(lidar_data,']')

                        lidar_points = []
                        lidar_points.append(lidar_data[open_brackets_indices[1]:closed_brackets_indices[0]])
                        lidar_points.append(lidar_data[open_brackets_indices[2]:closed_brackets_indices[1]])
                        lidar_points.append(lidar_data[open_brackets_indices[3]:closed_brackets_indices[2]])
                        lidar_points.append(lidar_data[open_brackets_indices[4]:closed_brackets_indices[3]])

                        # Making sure we are talking about the same point
                        assert lidar_points[3].split(',')[0].replace('[', '').replace('"', '').replace('\'', '') == timestamp_collision

                        for lidar_point in lidar_points:
                            splits = lidar_point.split(',')

                            print(f"truck FRONT \t\t\t{round(float(splits[13]), 2)}")
                            print(f"truck 45 \t\t{round(float(splits[17]), 2)}\t\t{round(float(splits[16]), 2)}")
                            print(f"truck sides \t\t{round(float(splits[15]), 2)}\t\t{round(float(splits[14]), 2)}")
                            print(f"")
                            print(f"trailer_0 \t\t{round(float(splits[1]), 2)}\t\t{round(float(splits[2]), 2)}")
                            print(f"trailer_1 \t\t{round(float(splits[3]), 2)}\t\t{round(float(splits[4]), 2)}")
                            print(f"trailer_2 \t\t{round(float(splits[5]), 2)}\t\t{round(float(splits[6]), 2)}")
                            print(f"trailer_3 \t\t{round(float(splits[7]), 2)}\t\t{round(float(splits[8]), 2)}")
                            print(f"trailer_4 \t\t{round(float(splits[9]), 2)}\t\t{round(float(splits[10]), 2)}")
                            print(f"trailer_5 \t\t{round(float(splits[11]), 2)}\t\t{round(float(splits[12]), 2)}")
                            print(f"------------------------------------------------------")
                        ...
                        if vehicle == 'truck':
                            ...
                            # self.player.set_transform(actor_transform)
                            # self.set_spectator_camera_view()
                            self.player = self.world.spawn_actor(blueprint, actor_transform)
                            self.world.tick()
                            time.sleep(5)
                            self.player.destroy()
                        if vehicle == 'trailer':
                            ...
                            # self.player.set_transform(actor_transform)
                            # self.set_spectator_camera_view()
                            self.player = self.world.spawn_actor(blueprintTrailer, actor_transform)
                            self.world.tick()
                            time.sleep(3)
                            self.player.destroy()
                            time.sleep(0.5)
