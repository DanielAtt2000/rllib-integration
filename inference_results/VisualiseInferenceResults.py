...
path = ('final/a3962295b7_1071ef25/testing')
...
uncompleted_left_routes  =0
uncompleted_right_routes = 0
trailer_uncompleted_routes_dict  = {}
truck_uncompleted_routes_dict  = {}
...
for route in routes:
    ...
    try:
        ...
        x['roundabout'], lane = get_route_type(int(x['route'].split('|')[0]),int(x['route'].split('|')[1]))

        if lane == 'left' and x['completed'] != 5:
            for i in range(5-x['completed']):
                uncompleted_left_routes+=1
        if lane == 'right' and x['completed'] != 5:
            for i in range(5-x['completed']):
                uncompleted_right_routes+=1
...
print(f'Uncompleted left routes {uncompleted_left_routes} = {uncompleted_left_routes/(uncompleted_left_routes+uncompleted_right_routes)}')
print(f'Uncompleted right routes {uncompleted_right_routes} = {uncompleted_right_routes/(uncompleted_left_routes+uncompleted_right_routes)}')

print(f'TRUCK uncompleted_routes_dict {truck_uncompleted_routes_dict}')
print(f'TRAILER uncompleted_routes_dict {trailer_uncompleted_routes_dict}')
